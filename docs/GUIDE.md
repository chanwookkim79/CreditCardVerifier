...
```
# 영수증 1건
logs/reports/20260412_cwkim_2600412001.txt

# 영수증 복수 건 (동일 전표번호, 영수증 N개)
logs/reports/20260412_cwkim_2600412001_01.txt
...
def process_single(
    ...
    #   3. 영수증 처리: ocr_items 있으면 바로 파싱, 없으면 JSON Mock / OCR
    #   4. 교차 검증 (R01~R15)
    #   5. 콘솔 출력 또는 JSON 출력
...
def process_email(
    ...
    # 이메일 1건 + 영수증 N건 처리 래퍼
    # 영수증 2건 이상이면 receipt_index=1,2,...로 process_single() 호출
    # 영수증 1건이면 receipt_index=None (접미사 없음)
    # SKIP(None) 결과 제외 후 list 반환

def main() -> None
    # argparse 처리
    # --receipt: nargs="+" (복수 경로 허용)
