...
SAP/NERP에서 내려오는 **신용카드 결제 승인 이메일**을 자동으로 분석하여, 첨부된 영수증 이미지와 교차 검증 후 15개 비즈니스 규칙(R01~R15)을 적용한다. 위반·주의 사항이 있으면 기안자에게 통보 이메일을 발송하고, 처리 결과를 CSV로 기록한다.
...
| step | PARSE / RECEIPT / CHECK / RESULT / NOTIFY 등 |
...
|------|:----:|------|---------|
| `OK` | ✅ | 정상 — 규정 준수 | OK |
| `WARN` | ⚠️ | 주의 — 개선 권고 (통보함) | WARN |
| `FAIL` | ❌ | 실패 — 규정 위반 (통보함) | FAIL |
| `SKIP` | ⏭️ | 건너뜀 — 해당 없음 | 공백 |
| `INFO` | ℹ️ | 참고 정보 | 공백 |
...
`config.py`의 `USE_MOCK_EMAIL`을 변경한다.

```python
USE_MOCK_EMAIL = True   # 콘솔 출력 (기본값, 테스트용)
USE_MOCK_EMAIL = False  # 실제 SMTP 발송 (운영용)
```
...
| 상수 | 기본값 | 설명 |
|------|--------|------|
| `USE_MOCK_EMAIL` | `True` | `True`=콘솔 출력 / `False`=SMTP 발송 |
...
### 8.11 `notifier/email_notifier.py`

```python
class EmailNotifier:

    def __init__(use_mock: bool = True, smtp_config: dict = None)

    def notify(result: CheckResult) -> None
        # use_mock=True  → 콘솔 출력
        # use_mock=False → SMTP 발송

    def _build_body(result: CheckResult) -> str
        # FAIL/WARN만 카테고리별 그룹화 (CATEGORY_ORDER 순서)
        # 인사말 + 위반 목록 + 요약 통계 포함

    def _mock_output(to_addr, subject, body) -> None
    def _send_smtp(to_addr, subject, body) -> None
```
...
신용카드 결제 점검 결과 아래와 같이 위반/주의 사항이 확인되었습니다.
이메일 ID: {email_id}  |  점검일시: {timestamp}

───────────────────────────────
【부가세】
  ❌ [규칙 01] 부가세 금액 확인
       부가세 불일치: 이메일 10,000원 ↔ 영수증 9,800원 (차이 200원, 허용 30원)

【원천세】
  ⚠️ [규칙 06] 선물 원천세 코드 확인
       품목당 150,000원 선물 - 원천세 코드 필수 (현재 없음)
───────────────────────────────

점검 요약: ✅ 정상 3건  ⚠️ 주의 1건  ❌ 실패 2건
```
...
def process_single(
    ...
    #   4. 교차 검증 (R01~R15)
    #   5. 콘솔 출력 또는 JSON 출력
    #   6. 위반 있을 때 통보 (no_notify=False 인 경우)
...
| `core/results_writer.py` | 인프라 | checker results.csv 기록 | `write_result()`, `_rule_status()`, `_notify_content()` |
| `core/__init__.py` | 패키지 | — | — |
| `master/master_data_loader.py` | 데이터 | 대행사·원천세 CSV 로드 및 조회 | `MasterDataLoader` |
| `master/__init__.py` | 패키지 | — | — |
| `notifier/email_notifier.py` | 통보 | Mock/SMTP 이메일 발송 | `EmailNotifier` |
...
다음 실행 시 파일이 없으면 헤더를 포함하여 자동 재생성된다.

### 9.6 SMTP 전환 체크리스트

운영 환경 전환 전 아래를 확인한다.

- [ ] `config.py`: `USE_MOCK_EMAIL = False`
- [ ] `notifier/email_notifier.py`: `smtp_config` 설정 (호스트, 포트, 계정, 비밀번호)
- [ ] `--no-notify` 없이 실행되는지 확인
- [ ] 테스트 주소로 Mock 수신 확인 후 실 운영 전환
